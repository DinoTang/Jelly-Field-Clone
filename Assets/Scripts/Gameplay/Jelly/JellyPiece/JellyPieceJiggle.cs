...
public class JellyPieceJiggle : JellyPieceAbstract
{
    ...
    protected override void Awake()
    {
        ...
        this.jellyVertices = new JellyVertex[this.meshClone.vertexCount];
        ...
        for (int i = 0; i < this.meshClone.vertexCount; i++)
        {
            ...
            this.jellyVertices[i] = new JellyVertex(i, worldPosition);
        }
    ...
    {
        if (this.meshClone == null || this.originalMesh == null || this.jellyVertices == null)
            return;
        ...
        {
            Vector3 target = this.GetDeformedTarget(originalVertices[i], movementX);

            this.jellyVertices[i].Shake(target, this.mass, this.stiffness, this.damping, maxStretch);
            ...
            float deformationWeight = this.GetDeformationWeight(originalVertices[i]);

            this.vertexArray[i] = Vector3.Lerp(originalVertices[i], localPosition, deformationWeight);
        }
    ...
    private Vector3 GetDeformedTarget(Vector3 originalVertex, float movementX)
    {
        ...
        float gripRadius = Mathf.Max(this.localBounds.size.x, this.localBounds.size.z) * 0.75f;
        ...
        float upperBodyLag = Mathf.SmoothStep(0f, 1f, height) * (1f - gripInfluence * 0.5f);
        ...
        lagOffsetX = Mathf.Clamp(lagOffsetX, -this.maxStretch, this.maxStretch);
    ...
    private float GetDeformationWeight(Vector3 originalVertex)
    {
        ...
        float bottomWeight = 1f - Mathf.SmoothStep(0f, 1f, height);

        return Mathf.Lerp(this.intensity, 1f, bottomWeight);
    }

    public void ResetJiggle()
    {
        if (this.originalMesh == null || this.meshClone == null || this.jellyVertices == null)
            return;
        ...
        for (int i = 0; i < this.jellyVertices.Length; i++)
        {
            ...
            this.jellyVertices[i].Reset(worldPosition);

            this.vertexArray[i] = originalVertices[i];
        }
